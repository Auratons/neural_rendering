...
class LayerConv(object):
    ...
    def __init__(
        self, name, w, n, stride, padding="SAME", use_scaling=False, relu_slope=1.0
    ):
        """Layer constructor.
        ...
        with tf.variable_scope(name):
            ...
            init_scale, pre_scale = init_he_scale(shape, relu_slope), 1.0
            if use_scaling:
...
class LayerTransposedConv(object):
    ...
    def __init__(
        ...
        with tf.variable_scope(name):
            ...
            init_scale, pre_scale = init_he_scale(kernel_shape, relu_slope), 1.0
            if use_scaling:
                init_scale, pre_scale = pre_scale, init_scale
            self._pre_scale = pre_scale
            self._weight = tf.get_variable(
    ...
    def __init__(
        ...
        with tf.variable_scope(self.name):
            ...
            with tf.variable_scope("res1"):
                self.blocks.append(
                    LayerPipe(
                        [conv2d("res1_conv"), norm_layer_constructor("res1_norm")]
                    )
                )
        ...
        with tf.variable_scope(self.name):
            ...
            with tf.variable_scope("bb_path2"):
                self.path2_blocks.append(
                    LayerPipe([downscale, conv2d("path2_conv", w=1, n=[nc_in, nc_out])])
                )
...
class LayerDense(object):
    ...
    def __init__(self, name, n, use_scaling=False, relu_slope=1.0):
        """Layer constructor.
        ...
        """
        with tf.variable_scope(name):
            init_scale, pre_scale = init_he_scale(n, relu_slope), 1.0
            if use_scaling:
                init_scale, pre_scale = pre_scale, init_scale
            self._pre_scale = pre_scale
            self._weight = tf.get_variable(

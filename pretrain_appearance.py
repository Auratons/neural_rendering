...
def get_triplet_input_fn(
    dataset_path, dist_file_path=None, k_max_nearest=5, k_max_farthest=13
):
...
def get_tf_triplet_dataset_iter(
    ...
    triplet_mapper = lambda idx: tuple(
        tf.py_func(
            get_triplet_input_fn(dataset_path, dist_file_path),
            [idx],
    ...
    if not deterministic_flag:
        ...
        triplet_dataset = triplet_dataset.map(
            triplet_to_dict_mapper, num_parallel_calls=None
        )
...
def build_model_fn(batch_size, lr_app_pretrain=0.0001, adam_beta1=0.0, adam_beta2=0.99):
    def model_fn(features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            optimizer = tf.train.AdamOptimizer(lr_app_pretrain, adam_beta1, adam_beta2)
            ...
            tf.train.init_from_checkpoint(
                osp.join(opts.train_dir), {"appearance_net/": "appearance_net/"}
            )
...
def train_appearance(train_dir, imageset_dir, dist_file_path):
    ...
    if resume_step != 0:
        tf.logging.warning("DBG: resuming apperance pretraining at %d!" % resume_step)

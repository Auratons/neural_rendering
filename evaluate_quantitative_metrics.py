...
FLAGS = flags.FLAGS
flags.DEFINE_string(
    "val_set_out_dir", None, "Output directory with concatenated fake and real images."
)
flags.DEFINE_string(
    "experiment_title", "experiment", "Name for the experiment to evaluate"
)
...
def compute_l1_loss_metric(image_set1_paths, image_set2_paths):
    ...
    total_loss = 0.0
    for ii, (img1_path, img2_path) in enumerate(
        zip(image_set1_paths, image_set2_paths)
    ):
...
def evaluate_experiment(val_set_out_dir, title="experiment", metrics=["psnr", "l1"]):
...
def main(argv):
    evaluate_experiment(
        FLAGS.val_set_out_dir, title=FLAGS.experiment_title, metrics=["psnr", "l1"]
    )

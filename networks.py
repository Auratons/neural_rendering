...
class PatchGANDiscriminator(object):
    def __init__(self, name_scope, input_nc, nf=64, n_layers=3, get_fmaps=False):
        ...
        conv2d = functools.partial(
            layers.LayerConv, use_scaling=opts.use_scaling, relu_slope=0.2
        )
        ...
        with tf.variable_scope(name_scope, tf.AUTO_REUSE):
            with tf.variable_scope("block_0"):
                self.blocks.append(
                    [conv2d("conv0", w=kw, n=[input_nc, nf], stride=2), activation]
                )
            for ii_block in range(1, n_layers):
                ...
                with tf.variable_scope("block_%d" % ii_block):
                    self.blocks.append(
                        [
                            conv2d(
                                "conv%d" % ii_block, w=kw, n=[nf_prev, nf], stride=2
                            ),
                            norm_layer(),
                            activation,
                        ]
...
class MultiScaleDiscriminator(object):
    def __init__(
        self, name_scope, input_nc, num_scales=3, nf=64, n_layers=3, get_fmaps=False
    ):
        self.get_fmaps = get_fmaps
        discs = []
...
class GeneratorPGGAN(object):
    def __init__(
        ...
        conv2d_rgb = functools.partial(
            layers.LayerConv, w=1, stride=1, use_scaling=use_scaling
        )
        ...
        with tf.variable_scope("g_model_dec", tf.AUTO_REUSE):
            ...
            for i in range(1, num_blocks + 1):
                with tf.variable_scope("res_%d" % i):
                    self.dec_stage.append(
                        layers.LayerPipe(
                            [
                                functools.partial(layers.upscale, n=2),
                                conv2d_stride1(
                                    "conv0", w=3, n=[multiply_factor * nf(i), nf(i + 1)]
                                ),
                                activation,
                                layers.pixel_norm,
                    ...
                    self.to_rgb.append(
                        conv2d_rgb("to_rgb", n=[nf(i + 1), opts.output_nc])
                    )
    ...
    def __call__(self, x, appearance_embedding=None, encoder_fmaps=None):
        ...
        enc_st_idx = 0
        if opts.use_appearance and opts.inject_z == "to_encoder":
            x = layers.tile_and_concatenate(
                x, appearance_embedding, opts.app_vector_size
            )
        ...
        if opts.use_appearance and opts.inject_z == "to_bottleneck":
            appearance_tensor = tf.tile(
                appearance_embedding, [1, tf.shape(y)[1], tf.shape(y)[2], 1]
            )
                ...
                self.blocks.append(
                    layers.LayerPipe(
                        [
                            ...
                            layers.BasicBlock(
                                "BB0", n=[64, 128], use_scaling=opts.use_scaling
                            ),
                            layers.pixel_norm,
                            layers.BasicBlock(
                                "BB1", n=[128, 192], use_scaling=opts.use_scaling
                            ),
                            layers.pixel_norm,
                            layers.BasicBlock(
                                "BB2", n=[192, 256], use_scaling=opts.use_scaling
                            ),
                            layers.pixel_norm,
                            activation,

#!/bin/bash
#SBATCH --job-name=evaluate_nriw_%j
...
echo "   --output_validation_dir=$(cat params.yaml | yq -r '.evaluate_nriw_'$sub'.output_validation_dir // (.evaluate_nriw_'$sub'.model_parent_dir + "../evaluations/" + .evaluate_nriw_'$sub'.dataset_name + "_'${TIMESTAMP}'_seq-'${VIRTUAL_SEQ_NAME}'")')"
...
~/.linuxbrew/bin/time -f 'real\t%e s\nuser\t%U s\nsys\t%S s\nmemmax\t%M kB' python $WORKSPACE/neural_rerendering.py \
    ...
    --output_validation_dir=$(cat params.yaml | yq -r '.evaluate_nriw_'$sub'.output_validation_dir // (.evaluate_nriw_'$sub'.model_parent_dir + "../evaluations/" + .evaluate_nriw_'$sub'.dataset_name + "_'${TIMESTAMP}'_seq-'${VIRTUAL_SEQ_NAME}'")') \

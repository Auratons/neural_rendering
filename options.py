...
flags.DEFINE_string(
    "dataset_parent_dir", "", "Directory containing generated tfrecord dataset."
)
flags.DEFINE_string("run_mode", "train", "{'train', 'eval', 'infer'}")
flags.DEFINE_string(
    "imageset_dir",
    None,
    "Directory containing trainset " "images for appearance pretraining.",
)
flags.DEFINE_string(
    "metadata_output_dir",
    None,
    "Directory to save pickled " "pairwise distance matrix for appearance pretraining.",
)
flags.DEFINE_integer(
    "save_samples_kimg",
    50,
    "kimg cycle to save sample" "validation ouptut during training.",
)

# Network inputs/outputs
flags.DEFINE_boolean("use_depth", True, "Add depth image to the deep buffer.")
flags.DEFINE_boolean("use_alpha", False, "Add alpha channel to the deep buffer.")
flags.DEFINE_boolean("use_semantic", True, "Add semantic map to the deep buffer.")
flags.DEFINE_boolean(
    "use_appearance", True, "Capture appearance from an input real image."
)
flags.DEFINE_integer(
    "deep_buffer_nc", 7, "Number of input channels in the deep buffer."
)
flags.DEFINE_integer(
    "appearance_nc", 10, "Number of input channels to the appearance encoder."
)
flags.DEFINE_integer("output_nc", 3, "Number of channels for the generated image.")

# Staged training flags
flags.DEFINE_string(
    "vgg16_path",
    "./vgg16_weights/vgg16.npy",
    "path to a *.npy file with vgg16 pretrained weights",
)
flags.DEFINE_boolean(
    "load_pretrained_app_encoder",
    False,
...
flags.DEFINE_boolean(
    "train_app_encoder",
    False,
    "Whether to make the weights " "for the appearance encoder trainable or not.",
)
flags.DEFINE_boolean(
    "load_from_another_ckpt",
    False,
    "Load weights from another trained model, "
...
# More hparams
flags.DEFINE_integer("train_resolution", 256, "Crop train images to this resolution.")
...
flags.DEFINE_integer(
    "num_crops", 30, "num crops from train images" "(use -1 for random crops)."
)
flags.DEFINE_integer("app_vector_size", 8, "Size of latent appearance vector.")
flags.DEFINE_integer(
    "total_kimg", 20000, "Max number (in kilo) of training images for training."
)
...
flags.DEFINE_float("w_loss_feat", 10.0, "Feature loss weight (from pix2pixHD).")
flags.DEFINE_float("w_loss_l1", 50.0, "L1 loss weight.")
flags.DEFINE_float("w_loss_z_recon", 10.0, "Z reconstruction loss weight.")
flags.DEFINE_float("w_loss_gan", 1.0, "Adversarial loss weight.")
flags.DEFINE_float("w_loss_z_gan", 1.0, "Z adversarial loss weight.")
...
flags.DEFINE_string("arch_type", "pggan", "Architecture type: {pggan, pix2pixhd}.")
flags.DEFINE_string(
    "training_pipeline", "staged", "Training type type: {staged, bicycle_gan, drit}."
)
flags.DEFINE_integer("g_nf", 64, "num filters in the first/last layers of U-net.")
flags.DEFINE_boolean(
    "concatenate_skip_layers", True, "Use concatenation for skip connections."
)

## if arch_type == 'pggan':
flags.DEFINE_integer("pggan_n_blocks", 5, "Num blocks for the pggan architecture.")
## if arch_type == 'pix2pixhd':
flags.DEFINE_integer(
    "p2p_n_downsamples", 3, "Num downsamples for the pix2pixHD architecture."
)
flags.DEFINE_integer(
    "p2p_n_resblocks",
    4,
    "Num residual blocks at the " "end/start of the pix2pixHD encoder/decoder.",
)
## if use_drit_pipeline:
flags.DEFINE_boolean("use_concat", True, '"concat" mode from DRIT.')
flags.DEFINE_boolean(
    "normalize_drit_Ez", True, "Add pixelnorm layers to the " "appearance encoder."
)
flags.DEFINE_boolean(
    "concat_z_in_all_layers",
    True,
    "Inject z at each " "upsampling layer in the decoder (only for DRIT baseline)",
)
flags.DEFINE_string(
    "inject_z",
    "to_bottleneck",
    "Method for injecting z; " "one of {to_encoder, to_bottleneck}.",
)
...
flags.DEFINE_string(
    "inference_input_path", "", "Parent directory for input images at inference time."
)
flags.DEFINE_string("inference_output_dir", "", "Output path for inference")
flags.DEFINE_string(
    "target_img_basename", "", "basename of target image to render for interpolation"
)
flags.DEFINE_string(
    "virtual_seq_name",
    "full_camera_path",
...
flags.DEFINE_string(
    "appearance_img1_basename",
    "",
    "basename of the first appearance image for interpolation",
)
flags.DEFINE_string(
    "appearance_img2_basename",
    "",
    "basename of the first appearance image for interpolation",
)
flags.DEFINE_list("input_basenames", [], "input basenames for inference")
flags.DEFINE_list(
    "input_app_basenames", [], "input appearance basenames for " "inference"
)
flags.DEFINE_string("frames_dir", "", "Folder with input frames to a camera path")
flags.DEFINE_string(
    "output_validation_dir",
    "",
    "dataset_name for storing results in a structured folder",
)
flags.DEFINE_string("input_rendered", "", "input rendered image name for inference")
flags.DEFINE_string("input_depth", "", "input depth image name for inference")
flags.DEFINE_string("input_seg", "", "input segmentation mask image name for inference")
flags.DEFINE_string(
    "input_app_rgb", "", "input appearance rgb image name for inference"
)
flags.DEFINE_string(
    "input_app_rendered", "", "input appearance rendered image name for inference"
)
flags.DEFINE_string(
    "input_app_depth", "", "input appearance depth image name for inference"
)
flags.DEFINE_string(
...
def validate_options():
    ...
    if not FLAGS.use_appearance:
        print(
            "**Warning: setting --app_vector_size to 0 since " "--use_appearance=False!"
        )
...
def list_options():
    configs = (
        "# Run flags/options from options.py:\n"
        "# ----------------------------------\n"
    )
    configs += "## Train flags:\n" "## ------------\n"
    configs += "train_dir = %s\n" % FLAGS.train_dir
    ...
    configs += "g_nf = %d\n" % FLAGS.g_nf
    configs += "concatenate_skip_layers = %s\n" % str(FLAGS.concatenate_skip_layers)

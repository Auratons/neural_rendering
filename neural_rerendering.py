    ...
    def model_fn(features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            if opts.training_pipeline == "staged":
                ops = staged_model.create_computation_graph(
                    x_in, x_gt, x_app=x_app, arch_type=opts.arch_type
                )
                ...
                return tf.estimator.EstimatorSpec(
                    mode=mode,
                    loss=ops["total_loss_d"] + ops["total_loss_g"],
                    train_op=train_op,
                )
            else:
                raise NotImplementedError(
                    "%s training is not implemented." % opts.training_pipeline
                )
...
def make_sample_grid_and_save(
    est, dataset_name, dataset_parent_dir, grid_dims, output_dir, cur_nimg
):
    ...
    utils.save_images(utils.to_png(utils.images_to_grid(images)), output_dir, cur_nimg)
...
def visualize_image_sequence(
    ...
    def seq_with_single_appearance_inp_fn():
        ...
        return {"conditional_input": x_in_op, "peek_input": x_app_tiled_op}
...
def train(
    ...
    if (
        opts.use_appearance
        and load_trained_fixed_app
        and not tf.train.latest_checkpoint(model_dir)
    ):
        tf.logging.warning(
            "***** Loading resume_step from %s!" % opts.fixed_appearance_train_dir
        )
        resume_step = utils.load_global_step_from_checkpoint_dir(
            opts.fixed_appearance_train_dir
        )
    else:
        tf.logging.warning("***** Loading resume_step (if any) from %s!" % model_dir)
    ...
    if opts.use_appearance and load_pretrained_app_encoder:
        tf.logging.warning(
            "***** will warm-start from %s!" % opts.appearance_pretrain_dir
        )
        ...
        tf.logging.warning(
            "****** finetuning will warm-start from %s!"
            % opts.fixed_appearance_train_dir
        )
        ws = tf.estimator.WarmStartSettings(
    ...
    est = tf.estimator.Estimator(
        model_fn, model_dir, config, params={}, warm_start_from=ws
    )

    for next_kimg in range(
        opts.save_samples_kimg, opts.total_kimg + 1, opts.save_samples_kimg
    ):
...
def evaluate_sequence(
    dataset_name, dataset_parent_dir, virtual_seq_name, app_base_path
):
...
def evaluate_image_set(
    dataset_name, dataset_parent_dir, subset_suffix, output_dir=None, batch_size=6
):
...
def _load_and_concatenate_image_channels(
    rgb_path=None, rendered_path=None, depth_path=None, seg_path=None, size_multiple=64
):
    """Prepares a single input for the network."""
    if (
        rgb_path is None
        and rendered_path is None
        and depth_path is None
...
def joint_interpolation(
    model_dir, app_input_dir, st_app_basename, end_app_basename, camera_path_dir
):
    """
    ...
    est_app = tf.estimator.Estimator(appearance_model_fn, model_dir, config, params={})
    ...
    for app_basename in [st_app_basename, end_app_basename]:
        ...
        app_in = _load_and_concatenate_image_channels(
            ...
            depth_path=app_depth_path,
            seg_path=app_seg_path,
        )
        # app_inputs.append(tf.convert_to_tensor(app_in))
        app_inputs.append(app_in)

    embedding1 = next(est_app.predict(lambda: {"peek_input": app_inputs[0]}))
    embedding1 = np.expand_dims(embedding1, axis=0)
    embedding2 = next(est_app.predict(lambda: {"peek_input": app_inputs[1]}))
    ...
    for frame_idx, embedding in enumerate(interpolated_embeddings):
        ...
        x_in = _load_and_concatenate_image_channels(
            rgb_path=None,
            rendered_path=frame_render_path,
            depth_path=frame_depth_path,
            seg_path=frame_seg_path,
        )

        img = next(
            est.predict(
                lambda: {
                    "conditional_input": tf.convert_to_tensor(x_in),
                    "appearance_embedding": tf.convert_to_tensor(embedding),
                }
            )
        )
        output_img_name = "%s_%s_%03d.png" % (
            st_app_basename,
            end_app_basename,
            frame_idx,
        )
...
def main(argv):
    ...
    if opts.run_mode == "train":
        ...
        evaluate_sequence(
            dataset_name, dataset_parent_dir, virtual_seq_name, inp_app_img_base_path
        )
    elif (
        ...
        evaluate_image_set(
            ...
            dataset_parent_dir,
            virtual_seq_name,
            opts.output_validation_dir,
            opts.batch_size,
        )
    elif (
        opts.run_mode == "eval_dir"
    ):  # evaluate output for a directory with input images
        ...
        joint_interpolation(
            model_dir, app_input_dir, st_app_basename, end_app_basename, frames_dir
        )
    else:

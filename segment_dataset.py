...
def _apply_colors(seg_images_path, save_dir, idx_to_color):
    for i, img_path in enumerate(seg_images_path):
        print(
            "processing img #%05d / %05d: %s"
            % (i, len(seg_images_path), osp.split(img_path)[1])
        )
...
def segment_images(
    ...
    if not osp.exists(xception_frozen_graph_path):
        raise OSError(
            "Xception frozen graph not found at %s" % xception_frozen_graph_path
        )
    ...
    with tf.Graph().as_default() as graph:
        new_input = tf.placeholder(
            tf.uint8, [1, crop_height, crop_width, 3], name="new_input"
        )
        tf.import_graph_def(
            graph_def,
    ...
    with tf.Session(graph=graph) as sess:
        for i, img_path in enumerate(images_path):
            ...
            if len(img.shape) == 2 or img.shape[2] != 3:
                ...
                continue
            img = utils.get_central_crop(
                img, crop_height=crop_height, crop_width=crop_width
            )
            img = np.expand_dims(img, 0)  # convert to NHWC format
            seg = sess.run("sem_seg/SemanticPredictions:0", feed_dict={new_input: img})

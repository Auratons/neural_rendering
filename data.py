...
def provide_data(
    ...
    record_parse_fn = functools.partial(
        _parser_rendered_dataset,
        crop_flag=crop_flag,
        crop_size=crop_size,
        use_alpha=opts.use_alpha,
        use_depth=opts.use_depth,
        use_semantics=opts.use_semantic,
        seeds=seeds,
...
def _parser_rendered_dataset(
    ...
    assert conditional_input.shape[-1] == opts.deep_buffer_nc, (
        "num channels "
    ...
    if crop_flag:
        ...
        if seeds is not None and len(seeds) <= 1:
            ...
            real = tf.image.resize_image_with_crop_or_pad(
                real, crop_size[0], crop_size[1]
            )
        else:
...
def multi_input_fn_record(
    ...
    assert len(filenames) > 0, (
        'Error! input pattern "%s" didn\'t match any ' "files" % input_pattern
    )
    ...
    if shuffle == 0:  # keep input deterministic
        ...
        dataset = dataset.map(
            record_parse_fn, num_parallel_calls=max(4, batch_size // 4)
        )

...
def build_dataset(
    ...
    """
    src_reference = str(Path(src) / "images")
    ...
    for i in range(10):
        ...
        if w >= 980 and h >= 546:
            if verbose:
                print("Processing image {}/{}".format(i + 1, len(H)), end="")

            # Reference image
            img = cv2.imread(os.path.join(src_reference, img_nm), cv2.IMREAD_UNCHANGED)
            print(f" img sz {img.shape}", end="")
            # img = get_central_crop(img, 546, 980)

            # depth image
            depth_map = read_array(
                Path(src) / "stereo" / "depth_maps" / f"{img_nm}.geometric.bin"
            )
            print(f" dm sz {depth_map.shape}")
            # depth_map = get_central_crop(depth_map, 546, 980)

            cv2.imwrite(os.path.join(out, "im_{:08n}.png".format(it)), img)
            plt.imsave(os.path.join(out, "dm_{:08n}.png".format(it)), depth_map)
            np.save(

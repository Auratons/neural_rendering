...
class AlignedRenderedDataset(object):
    def __init__(self, rendered_filepattern, use_semantic_map=True):
        ...
        assert len(self.filenames) > 0, (
            "input %s didn't match any files!" % rendered_filepattern
        )
        self.iter_idx = 0
    ...
    def next(self):
        if self.iter_idx < len(self.filenames):
            ...
            assert img_rendered.shape == (
                img_shape + (4,)
            ), "error in rendered " "image %s %s" % (basename, str(img_rendered.shape))
    ...
    for cur_dir in imgs_dirs:
        ...
        for ii, img_path in enumerate(images_path):
            ...
            if density <= ratio_threshold:
                ...
                print(
                    "filtered fie %d: %s with a desnity of %.3f"
                    % (ii, basename, density)
                )
